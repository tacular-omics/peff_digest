...
def test_digest_peff_file_no_crash(peff_path: Path) -> None:
    ...
    n_digested = 0
    with pf.PeffReader(str(peff_path)) as r:
        try:
            reader = iter(r)  # pefftacular >= 1.0 parses the header here
        except pf.PeffParseError:
            return  # invalid header (e.g. legacy '# PEFF' with no version): nothing to digest
        while True:
            try:
                entry = next(reader)
                ...
                continue  # a malformed entry ends the reader's generator; the next next() stops
