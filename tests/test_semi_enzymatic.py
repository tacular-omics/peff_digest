...
def test_semi_enzymatic_right_open() -> None:
    ...
    result = list(
        digest_peff_sequence(
            entry,
            _cfg(cleave_on="K", min_length=2, semi_enzymatic=True),
        )
    )
...
def test_semi_enzymatic_includes_fully_enzymatic() -> None:
    ...
    result = list(
        digest_peff_sequence(
            entry,
            _cfg(cleave_on="K", semi_enzymatic=True),
        )
    )
    seqs = {str(p.proforma) for p in result}
    assert "AAK" in seqs
...
def test_semi_enzymatic_with_missed_cleavages() -> None:
    ...
    result = list(
        digest_peff_sequence(
            entry,
            _cfg(cleave_on="K", missed_cleavages=1, semi_enzymatic=True, min_length=2),
        )
    )
